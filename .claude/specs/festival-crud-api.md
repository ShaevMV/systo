...
**Тело:**
```json
{ "data": { "id": "uuid?", "name": "Систо-Осень", "year": 2026, "active": true } }
```

**Валидация:**
| Поле | Правила |
|---|---|
| `data.id` | `sometimes, uuid` — опционально; можно задать с клиента, иначе генерируется на сервере |
...
```
`id` можно передать в `data.id` (валидный uuid); если не передан — генерируется на сервере (`Uuid::random()`). Ответ всегда содержит итоговый `id`.
