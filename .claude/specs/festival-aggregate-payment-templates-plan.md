...
### T4 — Спец-письмо внешних продавцов с адресом магазина ✅ Решено через AF-9 (2026-06-18, кода не требует)

> **Решение владельца (2026-06-18):** **структурные поля магазина НЕ нужны.** Под каждый тип оплаты (продавца) админ **создаёт отдельный email-шаблон** (редактор шаблонов AF-3) и **пишет адрес/контакты магазина прямо в теле шаблона**, затем привязывает его к типу оплаты (ось `types_of_payment_id`, AF-9). Адрес магазина = **контент шаблона**, не данные в БД.
> Поэтому миграция полей на `types_of_payment`, плейсхолдеры `PlaceholderCatalog` и проброс `vars` — **не требуются**. AF-10 закрыт реализацией AF-9 (T3) + существующим редактором шаблонов.
> **Остаётся только UI:** селектор «тип оплаты» в экране «Привязки шаблонов» (фронт, AF-8), чтобы создавать привязку продавец→шаблон из админки (сейчас доступно через API).

**Бизнес (контекст):** оплаты приходят от внешних продавцов/магазинов; для них нужно особое письмо с адресом этого магазина.

---

## Открытые вопросы (нужно подтверждение владельца)

1. ~~**«Адрес магазина»** — поля/URL/набор полей?~~ → **Решено (2026-06-18):** **структурных полей нет** — адрес/контакты магазина пишутся прямо в теле email-шаблона, созданного под тип оплаты. Это контент шаблона, не данные.
2. ~~**Привязка магазина** — к типу оплаты или к продавцу?~~ → **Решено (2026-06-18):** через **тип оплаты** (`types_of_payment_id`, ось AF-9). Отдельный шаблон под каждый тип оплаты.
...
**AF-10 закрыт (2026-06-18):** структурные поля магазина не нужны — адрес пишется в теле шаблона, привязанного к типу оплаты (AF-9). Остаётся только UI-селектор типа оплаты в экране привязок (фронт, AF-8).
