...
| **2. Repository + Events** | ✅ Готово | `d93d4c1a` | `InMemoryMySqlOrderTicketRepository`, 4 хендлера событий, `TicketApplication::createList` |
| **3. Controllers + Tests** | ✅ Готово | `8ecd095d` | FormRequest, `OrderTickets` (create/createFriendly), 5 Application-хендлеров, `PushQuestionnaireCommand`, сидеры, 5+ тестов. **PHPUnit 218/517 ассертов/0 ошибок.** Adversarial verify (code-reviewer + security-engineer) — критичных багов нет |

**🐛 Критический баг этапа 1, пойман на этапе 3:** при переписывании `OrderTicket.php` был **потерян `use ...OrderTicketDto;`** → все фабрики агрегата падали TypeError в рантайме (`php -l`/бутстрап не ловят — только при вызове метода). Поймал auto-tester на прогоне тестов, исправлено в `8ecd095d`. Урок: для Domain-рефакторинга статики мало, нужен прогон тестов/adversarial.

**Находки ревью (вынесены, НЕ блокируют merge):** см. TECH_DEBT — IDOR в `removeTicket` (pre-existing), `PriceService` мёртвый код после рефакторинга. Конфликт «общий FormRequest для create+createFriendly» (top-level поля Friendly не валидируются) — кандидат на отдельный `CreateFriendlyOrderRequest` в v2.6.1.
