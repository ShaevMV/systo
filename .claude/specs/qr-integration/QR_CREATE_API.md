...
> Маршрута **смены статуса (`changeStatus`) сейчас НЕ существует** (не зарегистрирован в роутах). Выдача билетов триггерится **только** через `/create` с оплаченным статусом.
