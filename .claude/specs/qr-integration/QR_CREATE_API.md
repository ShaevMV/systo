...
> **Связь с другими документами:** источник истины по приёму заказа — **этот документ**. Сопутствующие (существуют в этой папке): `HANDOFF_FOR_QR_AGENT.md` (шпаргалка), `ORDER_STATUSES_LIFECYCLE.md` (статусы заказа), `EMAIL_SEND_API.md` + `TEMPLATES_FOR_QR_AGENT.md` (письма и шаблоны), `QR_FULL_EXAMPLE.md` (полный пример контракта), `QR_INGEST_AUTH.md` (S2S-токен). _(Ранее упоминавшиеся `CONTRACT_RFC_v0.md` — отменённый переусложнённый дизайн с outbox/ассемблером — и `FUNCTION_MAP.md` **удалены/не существуют**, не искать их.)_
