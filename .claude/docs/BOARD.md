...
| **Ф5** | Offline-first PWA BazaFront (главный приоритет входа; бренд — со staging) | 🔄 **PR-1…PR-8 СДЕЛАНЫ, PR-9/PR-10 подготовлены** (автономная сессия PM 2026-06-21, ветка `feat/baza-f5-pr3-snapshot`, на стенде, Baza 122 теста зелёных, сборка BazaFront зелёная). Топология пересинхронена под **облако-мастер, ноут опционален** (встреча Q&A #13=A): истина=облако, телефон=тонкий клиент с шифрованным снимком + очередью намерений, синк по сотовой. PR-3 `/api/snapshot` (B5+ревью), PR-4 сканер (BarcodeDetector+светофор #16), PR-5 поиск без QR, PR-6 **гейт офлайна** (blacklist `/api/blacklist`+`/api/baza/ingest/revoke` — закрывает дыру отмена/возврат #8 + офлайн-PIN AES-GCM + шифрование снимка), PR-7 локальный SSL (`infra/kpp/`), PR-8 **гейт мульти-устройства** (журнал `baza_entry_events`, `/api/entry-events`, first-wins + дренаж). PR-9 (offline-узел) + PR-10 (cutover `infra/baza-cutover/`) — подготовлены, активация по гейтам. Боевой офлайн-впуск — только после PR-6 (#14). Спека: `.claude/specs/baza-f5-pwa.md`. |
