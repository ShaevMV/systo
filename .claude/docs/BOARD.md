...
| **Ф5** | Offline-first PWA BazaFront (главный приоритет входа; бренд — со staging) | 🔄 **В работе.** Дизайн-гейт снят (владелец 2026-06-20) + многоагентный архитектурный разбор (devops/архитектор/frontend/security) → PM принял 15 решений за владельца. Итог: переезжает ТОЛЬКО UI (Blade→PWA), Laravel-Baza = API-ядро; истина = локальный узел КПП, телефон — тонкий клиент с минимизированным шифрованным снимком + очередь намерений впуска; офлайн-вход = PIN (не кука); Strangler. **Карта PR-1…PR-10** (PR-8 append-only журнал = гейт мульти-устройства). **PR-1 (каркас+CI, закрывает TD-32) + PR-2 (service worker) стартуют сейчас**, PR-3+ ждут фактов феста. Спека: `.claude/specs/baza-f5-pwa.md`. |
