...
| **AF-7** | **Festival → AggregateRoot + история** — сейчас festival CRUD (создан 2026-06-18, см. ниже) — пассивная сущность под `Order/OrderTicket`. Сделать `Festival` полноценным агрегатом с `HasHistory` (события `festival_created/edited/deleted` в `domain_history`, `actor_type=user`) + `GET /api/v1/festival/getHistory/{id}` (admin). По образцу `Template`/`OrderTicket`. **Открытый вопрос:** выносить в новый модуль `Backend/src/Festival/` или AggregateRoot на месте. | M | новый backend-модуль | v2.7.0 / v2.8.0 | CRUD фестиваля (✅ done) |
...
| **AF-9** | **Привязка шаблонов писем/PDF по типу оплаты** — добавить ось `types_of_payment_id` в `template_bindings` (как ось `event`), резолвер учитывает payment-type, UI-селектор. Свести легаси `types_of_payment.email`/`pdf` (`emailPayView`) в единый механизм привязок. Расширение AF-3. | M | расширение AF-3 | v2.8.0 | AF-3 (✅ template_bindings) |
| **AF-10** | **Спец-письмо внешних продавцов с адресом магазина** — для оплат от внешних продавцов (`types_of_payment.user_external_id`) особое письмо с адресом магазина: данные магазина + плейсхолдеры адреса (`PlaceholderCatalog`) + спец email-шаблон по типу оплаты. | M | расширение AF-3/AF-6 | v2.8.0 | AF-9 |
