...
**Фронт-приложения в репо:** рядом со старым `FrontEnd/` (Vue CLI/webpack) теперь два новых Vite-приложения — `AdminFront/` (админка org, `/admin/`) и `BazaFront/` (offline-first PWA входа на КПП, Ф5, `/baza/` на `vhod.staging.spaceofjoy.ru`).

### Обновление Baza (Ф0–Ф8) — прогресс

org → admin-only: Baza становится offline-first системой контроля входа на КПП. План фаз: `.claude/specs/baza-update-implementation-plan.md`. Спека ролей: `.claude/specs/baza-f2-roles-rbac.md`. Память: `project_baza_entry_system`.

| Фаза | Что | Статус |
|------|-----|--------|
| Security-фикс впуска | `/api/scan`+`/api/enter` под auth, защита двойного впуска | ✅ PR #84 (merged) |
| **Ф0** | Локализация ассетов (офлайн-предусловие) | ✅ PR #88 (merged) |
| **Ф1** | Auth-дыры (`/users`,`/profile`) + удаление мусора + вынос паролей персонала из git | ✅ PR #89, #90 (merged) — проверено на стенде (0×500) |
| **Ф2** | Роли смены (`change_user`, 5 ролей) + RBAC-матрица через UI | ✅ **ЗАВЕРШЕНА — PR-1…PR-7 (#93/#94/#95/#97/#98/#99/#104) merged + на стенде** (vhod.staging, 0×500). VO ролей/действий, change_user (двойная запись), users.role, матрица `baza_role_permissions` + модуль Permission, permission-middleware (заменил IsAdmin), UI-редактор `/permissions`, выбор начальника смены в форме + инвариант «есть начальник» (PR-7). Идемпотентные сидеры стенда + демо-данные (TD-41, #101). ⚠️ Матрицу на стенде засеять force-seed'ом (сидеры теперь идемпотентны). |
| **Ф3** | ingest-API org→Baza (fallback прямая БД) | ✅ **PR #108 (Baza приём) + #109 (org маршрутизация) merged**. Baza: `POST /api/baza/ingest/ticket` (S2S `X-Baza-Token`, идемпотентно по UUID, цели el/spisok/live/auto, БД только в репозитории). org: `DeliverTicketToBazaJob` сперва ingest-API, при сбое — прямая запись (fallback). **Канал default OFF** (нет `BAZA_INGEST_URL`/`BAZA_INGEST_TOKENS`) → поведение не меняется. Тесты: Baza 9 + org 4 (+478/80 полные сьюты). На стенде роут отвечает 401 без токена (канал не активирован — нужны токены в .env обоих). |
| **Ф4** | Вебхук «билет прошёл» Baza→org | ✅ **PR #111 (Baza outbox) + #112 (org приём) merged**. Транспорт — HTTP-вебхук (решение владельца). Baza: `baza_entry_outbox` + запись при впуске (best-effort) + дренаж `baza:drain-entry-outbox` (schedule everyMinute). org: `POST /api/v1/baza/ticketEntered` (S2S `X-Baza-Token`, конфиг `baza_webhook`) → `domain_history` (`ticket_entered`, `actor_type=baza`, идемпотентно по `event_id`). **Канал default OFF** → впуск/поведение не меняются. ⚠️ Активация требует `schedule:run` на ноутбуке КПП (infra → TD). Тесты: Baza 7 + org 5. |
| **Поиск без QR** (rich-данные) | Поиск гостя по всем полям, когда нет работающего QR | ✅ **PR #113 (Baza ticket_search) + #114 (org обогащение) merged** (запрос владельца). Локальная таблица-индекс `ticket_search` (поиск ОФЛАЙН + в синке → на ноутбук КПП); наполняется из ingest богатым блоком `search` (fio/phone/telegram/car/child/external_no), org обогащает через `search_blob`. Поиск по ФИО/телефону/telegram/госномеру/имени ребёнка/№ заказа + секция в `/search` «Найдено по данным (без QR)» с мостом к впуску. Демо-сидер `TicketSearchTestDataSeeder`. Тесты: Baza 6 + org 3. ⚠️ Требует ingest-канал ON. |
| **Ф5** | Offline-first PWA BazaFront (главный приоритет входа; бренд — со staging) | 🔄 **PR-1…PR-8 СДЕЛАНЫ + RBAC/смены/персонал/поиск + org→Baza ingest активирован** (сессии PM 2026-06-21, на стенде + в master `#127`/`#128`, Baza 161 тест зелёный). Топология — **облако-мастер, ноут опционален** (Q&A #13=A): истина=облако, телефон=тонкий клиент с шифрованным снимком + очередью намерений, синк по сотовой. PR-3 `/api/snapshot`, PR-4 сканер (BarcodeDetector+светофор), PR-5 поиск без QR (по всем полям), PR-6 **гейт офлайна** (blacklist + офлайн-PIN AES-GCM + шифрование снимка), PR-7 локальный SSL (`infra/kpp/`), PR-8 **гейт мульти-устройства** (журнал `baza_entry_events`, first-wins). Доп-серия: `/api/whoami` + ПДн-фильтр (право `ticket.pii`), экраны Права/Регистрация/Смены в PWA (`/baza/`), сидеры RBAC, org→Baza через ingest-API (стенд). PR-9 (offline-узел) + PR-10 (cutover) — подготовлены, активация по гейтам. Спека: `.claude/specs/baza-f5-pwa.md`. |
| **Ф6** | Браслеты↔билет + парковки (4 типа) | ⏳ |
| **Ф7** | Финансы и матответственность (гейт Q4) | ⏳ |
| **Ф8** | Синк новых таблиц + read-витрина смен в org | ⏳ |

**Решения владельца:** RBAC-матрица — 2 оси (роль×действие), готовит дизайн-контракт под незаконченную org-матрицу `.claude/specs/admin-rbac.md` (без cross-schema). Линтеры CI → **TD-38**, Swagger/OpenAPI → **TD-39** (для интеграции).
...
| **AF-9** | **Привязка шаблонов писем/PDF по типу оплаты** — ✅ **сделан полностью (backend 2026-06-18 + UI 2026-06-19):** ось `types_of_payment_id` в `template_bindings`, резолвер (вес 16 — сильнейший override), проброс в выдачу билета (email/PDF), легаси `types_of_payment.email` — fallback. **UI:** селектор «Тип оплаты» в экране «Привязки шаблонов» (`TemplateBindingListView` + `TemplateBindingModule`) — форма/колонка/проброс, e2e подтверждён на стенде. PR #80 (`feat/qr-order-projection`). | M | расширение AF-3 | ✅ done | AF-3 (✅ template_bindings) |
...
### 2026-06-23 — Изоляция КПП по фестивалю смены (TD-48) + реестр фестивалей + Орг-CRUD

**Baza видит на КПП ТОЛЬКО билеты фестиваля открытой смены. За флагом `BAZA_FESTIVAL_ISOLATION` (default OFF). Катится ВПЕРЁД RabbitMQ (решение владельца).**

- 🌿 Ветка: `feat/festival-service` (от master), коммит `dec637e6`, на staging (флаг ON, проверено вживую). PR в master — pending.
- 📜 Память: `project_festival_isolation_2026_06_23`. Закрывает **TD-48**.

**Что вошло (7 «PR» + Орг-экран):**
- **Реестр фестивалей в Baza** (`scr/Festival/`, таблица `festivals`, CQRS, идемпотентный сидер, право `festival.manage`) — реплика каталога org.
- **Смена несёт `festival_id`** (выбор при открытии: авто-выбор единственного / обязателен при нескольких; селектор в PWA `ShiftsView` + legacy Blade).
- **request-scoped `FestivalScope`** (singleton) вместо зашитой константы `9d679bcf-…` в 7 ticket-репо. Скан/поиск/впуск/счётчики-отчёт/офлайн-снимок изолируются по фестивалю **открытой смены**.
- **Скан чужого билета** → жёлтый вердикт «другой фестиваль»; **впуск чужого блокируется**, override по праву `entry.override_festival` (скоупится фестивалём билета — без коллизии kilter).
- **Закрыты дыры live/parking** (`festival_id` + миграция `2026_06_23_130000` + backfill + lenient-фильтр; live получает festival из связанного el при ingest).
- **Отчёт смен по фестивалю** (де-хардкод + селектор).
- **fail-closed:** при ON без смены / с пустым `festival_id` смены снимок/поиск/впуск НЕ отдают дефолтный фестиваль (защита ПДн).
- **Орг (AdminFront):** экран CRUD фестивалей + история (`FestivalListView` + `FestivalModule`).
- При **OFF** поведение идентично прежнему (зашитый UUID → `config('baza.default_festival_id')`).

**Решения владельца:** реестр-реплика из org (мастер — org); одна смена = один фестиваль; чужой билет = жёлтый + блок впуска (override по праву); фестивали катятся вперёд RabbitMQ; сначала проверка на стенде под флагом → потом PR в master.

**Проверка:** PHPUnit Baza **212/212** (39 Festival-тестов, включая fail-closed), BazaFront/AdminFront собираются. Деплой на staging успешен; изоляция **проверена вживую** на реальных данных (66 своих / 4+5 чужих билетов): свой → впуск, чужой → блок, fail-closed → пусто, фестиваль смены резолвится; эндпоинты с флагом ON без 500.

> ⚠️ **Staging ротирован с rabbitmq на festival** (rabbitmq забэкаплен в `origin/staging-rabbitmq-backup`). rabbitmq переребейзится на master после мержа фестиваля.

### 2026-06-20 — Ф5 PWA BazaFront: каркас + service worker (PR-1, PR-2)

**Стартовал фронт Ф5: offline-first PWA входа на КПП. Новое Vite-приложение `BazaFront/` рядом со старым Blade-UI Baza (Strangler). На стенде `https://vhod.staging.spaceofjoy.ru/baza/`.**

- 🌿 Приложение: `BazaFront/` (Vite + Vue 3 + PrimeVue + vite-plugin-pwa/Workbox)
- 📜 Спека: `.claude/specs/baza-f5-pwa.md`

**Что вошло:**
- **PR-1 (#120/#121) — каркас + инфра + CI.** Каркас `BazaFront/` (Vite+Vue3+PrimeVue, роутер, session-куки+CSRF, не JWT), Docker-сервис `node-baza-staging`, nginx `location /baza/` (server-блок `vhod.*`, `--base=/baza/`), CI-job `build-baza-front` в `ci.yml` (**закрыл TD-32** для этого фронта — PWA входа собирается в CI заранее), шаг Vite-сборки в `deploy-staging.yml`. На стенде `/baza/` → 200.
- **PR-2 (#122) — service worker + офлайн-оболочка.** Service worker (Workbox `registerType:'prompt'`) + precache app-shell, `manifest.webmanifest` (standalone/install), версия в шапке, **IndexedDB append-only очередь намерений впуска** (`BazaFront/src/db/queue.js`: store `queue` — не редактируем чужие записи, только добавляем; основа мульти-устройственной синхронизации PR-8) + `device_id`. Оболочка открывается офлайн.

**Топология (скорр. владельцем 2026-06-20):** ноут/узел КПП **опционален** (его может не быть); eventual-истина = облако (боевой сервер), синк телефон↔облако по любому интернету (сотовая); на полный блэкаут консистентность держит **физический браслет** (anti-double-entry, Ф6); кросс-браузерный PWA; смена 10ч.

**Дальше:** PR-3+ (минимизированный шифрованный снимок билетов для IndexedDB, экраны сканера/поиска/смены, офлайн-PIN, append-only журнал `baza_entry_events`) — ждут фактов феста. Бэкенд-довесок Ф5 (snapshot-API, журнал) ещё не реализован — см. TD-46.
