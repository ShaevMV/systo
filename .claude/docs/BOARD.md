...
| **Ф5** | Offline-first PWA BazaFront (главный приоритет входа; бренд — со staging) | 🔄 **В работе.** Дизайн-гейт снят (владелец 2026-06-20) + многоагентный архитектурный разбор (devops/архитектор/frontend/security) → PM принял 15 решений за владельца. Итог: переезжает ТОЛЬКО UI (Blade→PWA), Laravel-Baza = API-ядро. **Топология (скорр. владельцем 2026-06-20):** ноут/узел КПП **опционален** (его может не быть, не единственный источник истины); eventual-истина = облако (боевой сервер), синк телефон↔облако по любому интернету (сотовая, не только локальный Wi-Fi); телефон автономен офлайн (минимизированный шифрованный снимок + append-only очередь намерений впуска); офлайн-вход = PIN (не кука); на полный блэкаут консистентность держит **физический браслет** (anti-double-entry, Ф6); кросс-браузерный PWA; смена 10ч; Strangler. **Карта PR-1…PR-10** (PR-8 append-only журнал `baza_entry_events` = гейт мульти-устройства). **PR-1 (каркас BazaFront/ + Docker node-baza + nginx `/baza/` + CI-job `build-baza-front`, закрыл TD-32) ✅ merged + на стенде (`https://vhod.staging.spaceofjoy.ru/baza/` → 200); PR-2 (service worker Workbox prompt + precache app-shell + manifest install + IndexedDB append-only очередь намерений + device_id) ✅ merged.** PR-3+ (снимок билетов, экраны сканера/поиска/смены, офлайн-PIN, журнал `baza_entry_events`) — ждут фактов феста. Спека: `.claude/specs/baza-f5-pwa.md`. |
