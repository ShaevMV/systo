...
| **Ф2** | Роли смены (`change_user`, 5 ролей) + RBAC-матрица через UI | ✅ **PR-1…PR-6 (#93/#94/#95/#97/#98/#99) merged + на стенде** (vhod.staging, 0×500). VO ролей/действий, change_user (двойная запись), users.role, матрица `baza_role_permissions` + модуль Permission, permission-middleware (заменил IsAdmin), UI-редактор `/permissions`. Остаётся **PR-7** (refinement): явные роли участникам в форме смены + инвариант «есть начальник» (контракт SaveChange, отложено из PR-2). ⚠️ На стенде матрица пустая (auto-seed пропускает непустую БД) — админы проходят через суперроль; нужен понятный сидер (см. TD-41). |
| **Ф3** | ingest-API org→Baza (fallback прямая БД) | ⏳ |
