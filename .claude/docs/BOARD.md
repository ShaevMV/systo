...
| **2.6.0** | 🔥 **до 2026-06-12** | Минимум для старта продаж: опции к билетам + новый формат заказа (BREAKING) + миграция + фронт + тесты | 🔥 В работе |
| **2.7.0** | июль 2026 | Промокоды-агрегатор (история, права на все роли кроме guest) + qr.spaceofjoy.ru SSO (Passport hybrid с JWT) | ⏳ Запланировано |
| **2.8.0** | начало сентября 2026 | SSL для ноутбука-сканера, offline docker-compose, CD staging | ⏳ Запланировано |
| **2.9.0** | сентябрь–октябрь 2026 | Loki + Grafana, audit + воронка покупки, CD prod tag-based | ⏳ Запланировано |
| **3.0.0** | TBD | Laravel 11 + OrderKind VO + 3 Application-сервиса | 🟡 Открытый |

**🔥 Дедлайн 2026-06-12** — старт продаж осеннего фестиваля. **Scope v2.6.0 сокращён** после анализа 3 спецификаций — промокоды-агрегатор и Passport SSO перенесены в v2.7.0. Источник: `.claude/meetings/2026-05-30/RESULTS.md` + спеки в `.claude/specs/`.

---

## 🔄 Текущая работа

**🔥 Спринт v2.6.0 (L) — дедлайн 2026-06-12 (старт продаж осеннего фестиваля).**

Интеграционная ветка `feat/v2.6.0-fall-festival` + 6 sub-веток. Scope **сокращён** после анализа 3 спецификаций — промокоды-агрегатор и Passport SSO перенесены в v2.7.0.

### Sub-ветки

| Ветка | Что | Оценка |
|-------|-----|--------|
| `feat/v2.6.0-option-entity` | Модуль `Option` + миграция + CRUD + админ UI | M |
| `feat/v2.6.0-order-domain-rewrite` | VO `OrderGuestLine`, `Money`, переделка `OrderTicket`, `OrderPriceCalculator` | L |
| `feat/v2.6.0-order-data-migration` | Миграция БД + перенос существующих заказов (с бэкапом) | M |
| `feat/v2.6.0-controllers-rewrite` | Обновление контроллеров под новый формат (legacy убираем сразу) | M |
| `feat/v2.6.0-buy-form-rewrite` | `BuyTicket.vue` + live-расчёт цены | M |
| `feat/v2.6.0-tests` | PHPUnit + E2E + регресс на staging | S |

### Зафиксированные решения (2026-05-30)

1. **Промокод процентный** считается только от базового билета (не от опций)
2. **Кратность опций** через `qty`: `options: [{option_id, qty: 2}]`
3. **Опции в `createList`** — НЕ нужны (списки бесплатные)
4. **Live + non-live в одном заказе** — нельзя (валидация «всё одного типа»)
5. **Legacy формат API** — НЕ поддерживаем, сразу BREAKING
6. **Passport** — в v2.7.0, hybrid с JWT (не full migration)

### Спецификации (готовы)

- `.claude/specs/ticket-options.md` (business-analyst, 579 строк)
- `.claude/specs/order-format-architecture.md` (tech-lead, 719 строк)
- `.claude/specs/qr-sso-security.md` (security-engineer, 562 строки)

### Спринт v2.7.0 (после v2.6.0)

| Блок | Что |
|------|-----|
| Промокоды-агрегатор | История + `created_by_user_id` + права на все роли кроме `guest` |
| qr.spaceofjoy.ru SSO | Laravel Passport hybrid с JWT (новый guard `api-oauth`), роль `qr_service`, consent screen для 152-ФЗ |
