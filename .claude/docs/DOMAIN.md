...
| DTO | Файл | Поля |
|-----|------|------|
| **QrOrderDto** | `QrOrder/Dto/` | `id` (Uuid, == id заказа qr/org), `email`, `status`, `festivalId` (?Uuid), `typeOrder` (?string: regular/friendly/list/live), `city`, `phone`, `totalPrice` (int, рубли), `payload` (array — весь контракт qr), `issuedAt`, `externalOrderNo` (?string), `paymentMethod` (?string), `promoCode` (?string), `paidAt` (?Carbon), `buyerFio` (?string), `festivalTitle` (?string). Фабрики: `fromState($row)`, `fromQrContract($json)` (проецирует `external_order_no`, `payment.method`, `payment.promo_codes[0]`, `order_data.paid_at`; **расширенный контракт 2026-06-20:** `buyer.fio`→`buyerFio` (fallback `user.name`), `buyer.city/phone` (fallback `user.*`), `payment.amount_total`→`totalPrice` (fallback `price.total`), `order_data.festival.title`→`festivalTitle`; весь JSON в `payload`; гард `MAX_GUESTS=1000`). Новые проекции — named-параметры в конце конструктора (защита от смещения позиционных) |
...
| Метод | Описание |
|-------|----------|
| `create(BazaDeliveryDto, ?subjectBlob, ?searchBlob): bool` | Создать запись доставки (`queued`). Одна строка на `(ticket_id, target)` — UNIQUE. Наряду с записью персистит `subject_blob` (готовый `TicketResponse` для el/spisok) и `search_blob` (богатые поля гостя для индекса `ticket_search` Baza «поиск без QR»). Доставка читает их через `getSubjectBlob`/`getSearchBlob` и подмешивает `search` в ingest-payload (`ingestOrDirect`) |
