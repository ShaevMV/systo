...
| TD-37 | **Legacy org-письма идут мимо `MailDispatcher`** — после системы писем (Ф1–Ф5) трекинг (`email_messages`) пишут только qr-каналы (`source` = `qr_pipeline` / `qr_intake`) через `MailDispatcher::send()`. Старые org-уведомления (отмена/изменение/трудности через `Order/OrderTicket/Domain/ProcessUserNotification*` → `Mail::to()->send()`) **диспетчер минуют** и в админке «Доставка писем» **не видны**. Спека планировала passive-listener на `MessageSending`/`MessageSent` (расширить `App\Listeners\LogSentMessage`, `source='org_event'`), но **он не реализован** — `LogSentMessage` по-прежнему пишет только в монолог (`Log::info`), не в `email_messages`. Решение: подключить org-события к диспетчеру (или включить listener), чтобы legacy-письма попадали в `email_messages` (status `sent`, `source='org_event'`). org-публичный flow deprecated по pivot — полная миграция org-событий отдельной фазой. | backend | После стабилизации системы писем |
