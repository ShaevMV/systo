...
| TD-48 | **Baza: изоляция смены по фестивалю** (запрошено владельцем 2026-06-21, **🔴 на потом**) — в Baza нужна система работы с фестивалями: **смена привязана к фестивалю**, и одна смена на одном фестивале видит **ТОЛЬКО билеты в рамках этого фестиваля** (изоляция выборки по `festival_id` — скан/поиск/впуск/счётчики смены не должны показывать билеты других фестивалей). Контекст: в Baza уже есть `festival_id` в проекциях билетов (`el_tickets.festival_id` и пр., миграции v2.5.1 + rich-данные), смены (`change_user`/Ф2 роли) и поиск без QR (`ticket_search`). Нужно: связать смену с фестивалем (выбор фестиваля при открытии смены?), прокинуть фильтр `festival_id` во все выборки билетов на КПП (скан, поиск, отчёт смены) + в офлайн-снимок PWA (Ф5, чтобы на устройство грузились билеты только своего фестиваля). **Уточнить семантику у владельца перед стартом** (одна активная смена = один фестиваль; что при мультифестивальном билете). Не обсуждалось детально — записано как TODO на потом. | backend + tech-lead | **На потом (после RabbitMQ/Grafana)** |
