...
| TD-36 | **Email-доставка билетов без подтверждения (AF-6)** — сейчас «слепой SMTP»: `SendOrderEmailStep` → `Mail::to()->send()`, лог `send_order_email.sent` = письмо принято SMTP-сервером, но **доставка/открытие/bounce не отслеживаются** (трекинга в коде нет). Нельзя узнать, получил ли пользователь билет на почту. Нужен транзакционный email-провайдер с вебхуками (`delivered`/`bounced`/`opened`) — под РФ/152-ФЗ: UniOne/Unisender, SendPulse, Mailopost — + таблица статусов писем + экран «Доставка на почту» + повторная отправка. Пара к AF-4 (baza). Нужен аккаунт провайдера (биллинг владельца). | backend + devops-engineer | v2.8.0 (AF-6) |
