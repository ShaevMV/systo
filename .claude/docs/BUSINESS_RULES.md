...
| `curator_participant` | Анкета участника куратора | 3 поля (имя, контакт, фото). Тип анкеты определяется по `location.questionnaire_type_id` |
...
### Правила анкеты участника куратора (curator_participant)

- Каждая локация имеет собственный тип анкеты (`location.questionnaire_type_id`)
- При создании заказа куратора (`createList`) для каждого участника с email создаётся пользователь через `AccountApplication::creatingOrGetAccountId`
- При переводе заказа в статус `pending_curator` — участники получают ссылки на анкеты своих локаций по email
- Константы шаблонов в `OrderTicket`: `CURATOR_LIST_TICKET_PDF = 'curator_list'`, `CURATOR_LIST_EMAIL_CODE = 'curator_list'`
