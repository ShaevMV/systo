...
- **Поддержано два режима жизненного цикла:**
  - **Двухшаговый:** `qrOrder/create` со `status = «создан»` (`создан`/`new`/`created`) → org шлёт письмо **«заказ создан»** (шаблон `orderToCreate`, `source = qr_pipeline`), билеты **НЕ выпускает**. Затем `qrOrder/changeStatus` → `«оплачен»` → **выдача билетов один раз**.
  - **Одношаговый:** `qrOrder/create` сразу со `status = «оплачен»` → **немедленная выдача билетов**.
  - Письмо «заказ создан» шлётся **только для `type_order = regular`** (заказы-списки писем при создании не шлют; живые билеты стартуют оплаченными — письма «создан» для них нет, как в классическом org-флоу).
- При смене статуса в `оплачен`/`paid` (и `issued_at == null`) запускается **выдача билетов** (PDF/письма) — **один раз**, защита по `issued_at` (повторный «оплачен» от ретраев qr не выдаёт билеты снова).
