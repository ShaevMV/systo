<template>
  <div class="qr-orders-view">
    ...
    <Dialog
      ...
      modal
      :header="'Заказ ' + (item.id || '')"
      :style="{ width: '60rem' }"
      :breakpoints="{ '960px': '95vw' }"
    >
      <div class="qr-details">
        <div class="qr-details-grid">
          <div><span class="qr-label">Статус</span>
        ...
        <div v-if="orderData.comment" class="qr-comment">
          <span class="qr-label">Комментарий</span> {{ orderData.comment }}
        </div>
        ...
        <Timeline :value="history" class="qr-timeline">
          <template #content="{ item: ev }">
            <div class="qr-history-event">
              ...
              <span v-if="ev.payload?.from || ev.payload?.to" class="qr-history-transition">
...
const festivalOptions = computed(() =>
  festivals.value.map((f) => ({ label: `${f.name}${f.year ? ' ' + f.year : ''}`, value: f.id })),
);
