...
Route::prefix('v1/order')->group(static function (): void {
    ...
    Route::post('/changeTicket/{id}', [OrderTickets::class, 'changeTicket'])
        ->middleware('auth:api')
        ->middleware('role:admin,pusher,pusher_curator,curator');
    ...
    // Тред комментариев заказа (C1): добавить + прочитать весь тред (admin/manager —
    // кто пишет, тот и читает свой тред).
