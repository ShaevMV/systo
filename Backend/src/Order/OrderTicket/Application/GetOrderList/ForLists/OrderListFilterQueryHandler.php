...
class OrderListFilterQueryHandler implements QueryHandler
{
    ...
    public function __invoke(OrderFilterQuery $filterQuery): ?ListResponse
    {
        ...
        $orderTicketItems = $this->orderTicketRepository->getListsList($filter);

        $result = [];
        foreach ($orderTicketItems as $value) {
            $result[] = $value->setGuests(
                $value->getGuestsByFestivalId($filterQuery->getFestivalId())
            );
        }
    ...
    private function getFilterValues(OrderFilterQuery $filterQuery): array
    {
        $result = [
            ...
            [
                'field' => OrderTicketModel::TABLE . '.location_id',
                'operator' => FilterOperator::EQUAL,
                'value' => $filterQuery->getLocationId()?->value(),
            ],
            [
        ...
        if ($filterQuery->getCuratorId() !== null) {
            $result[] = [
                'field' => OrderTicketModel::TABLE . '.curator_id',
                'operator' => FilterOperator::EQUAL,
                'value' => $filterQuery->getCuratorId()->value(),
            ];
        }
