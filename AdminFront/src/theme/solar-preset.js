...
// DARK surface — фирменный космос, но КОМФОРТНО средне-тёмный (не «провал в чёрное»).
// Подтон сохраняем сливово-фиолетовый/синий (плам #311332 + навигатор), однако нижнюю
// часть шкалы (700…950) заметно осветлили: фон/карточки/сайдбар теперь читаются как
// «глубокий сумеречный фиолет», а не «почти чёрный». Калибровка после фидбэка владельца
// («сделай чуть светлее, уж очень темно»).
const surfaceDark = {
    0: '#ffffff',
    50: '#f6f3f7',
    100: '#e8e0ea',
    ...
    400: '#8f7a98',
    500: '#6e5c78',
    600: '#5a4a66', // приподнятые карточки/оверлеи (popover, hover)
    700: '#4a3b58', // боковая панель / surface-overlay
    800: '#3e3050', // основной контент / карточки (на тон светлее фона)
    900: '#33263f', // фон страницы (body) — сумеречный плам, не чёрный
    950: '#241a30' // самый глубокий тон (для редких акцентов/код-блоков)
};
    ...
    semantic: {
        ...
        colorScheme: {
            ...
            dark: {
                ...
                content: {
                    // Карточки/контент — surface-800 (на тон светлее фона-900): мягкая «приподнятость».
                    background: '{surface.800}',
                    // Границы чуть заметнее на осветлённом фоне — таблицы/инпуты лучше очерчены.
