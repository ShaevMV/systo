...
export const removeToken = (state) => {
    ...
    state.userInfo = { id: null, email: null, admin: false, manager: false, seller: false, pusher: false, curator: false, pusherCurator: false };

    // Удаляем только ключи пользователя, не трогая остальной localStorage
    ['user.token', 'user.token.lifetime', 'user.email', 'user.id', 'user.isAdmin', 'user.isManager', 'user.isSeller', 'user.isPusher', 'user.isCurator', 'user.isPusherCurator', 'user.role'].forEach((key) => localStorage.removeItem(key));
};
...
export const setUserInfo = (state, payload) => {
    ...
    state.userInfo = {
        ...
        pusherCurator: role === 'pusher_curator'
    };
    ...
    localStorage.setItem('user.isPusherCurator', String(state.userInfo.pusherCurator));
