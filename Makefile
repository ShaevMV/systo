...
.PHONY: up
up: ## Поднять dev-окружение (docker-compose.yml)
	$(DC) up -d --build
...
.PHONY: up-prod
up-prod: ## Поднять prod-окружение (docker-compose.prod.yml)
	$(DC_PROD) up -d --build
...
.PHONY: build
build: ## Сборка фронтенда (production)
	$(NODE) npm run build

.PHONY: dev
dev: ## Dev-сервер фронтенда
	$(NODE) npm run dev
