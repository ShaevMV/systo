...
- **Система шаблонов (AF-3), фазы 1–5** — единый редактор писем и PDF-билетов. Спека: `.claude/specs/template-system.md`.
  ...
  - Импорт текущих blade в БД (`artisan templates:import-blade`, неактивные черновики) + конвертация в Mustache (`artisan templates:sync-converted`, фаза 5 — **28 шаблонов активны на стенде**: все 20 order/list писем + 7 PDF-билетов + базовый `pdf`; глубокий рендер-тест 133 assertions + e2e).
