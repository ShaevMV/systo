...
- **Система шаблонов (AF-3), фазы 1–5** — единый редактор писем и PDF-билетов. Спека: `.claude/specs/template-system.md`.
  ...
  - Импорт текущих blade в БД (`artisan templates:import-blade`, неактивные черновики) + конвертация в Mustache (`artisan templates:sync-converted`, фаза 5 — **32 шаблона активны на стенде**: 20 order/list писем + 7 PDF-билетов + базовый `pdf` + 4 auth/анкетных; глубокий рендер-тест 154 assertions + полный сьют 318 + e2e).
